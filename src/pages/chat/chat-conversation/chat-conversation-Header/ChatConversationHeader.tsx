...
interface ChatConversationHeaderProps {
  ...
  selectedChat: any;
  // setSelectedChat: React.Dispatch<React.SetStateAction<any>>; // Define the type of setSelectedChat if possible
}
...
const ChatConversationHeader: React.FC<ChatConversationHeaderProps> = ({ conversationInfos, selectedChat }) => {
  ...
  const handleBlockFriend = async () => {
    ...
    if (selectedChat) {
      try {
            const response = await fetch(`http://localhost:3001/friend/${selectedChat.user_id}/block/${selectedChat.friend_id}`, {
            method: "POST", 
          });
      ...
      <div className="chat-conversation-infos">
        <div className="chat-conversation-images">
          {conversationInfos?.images?.map((image: any, index: number) => (
            <img src={process.env.PUBLIC_URL + image} alt="member avatar" key={`key-${index}`} />
          ))}
        </div>
