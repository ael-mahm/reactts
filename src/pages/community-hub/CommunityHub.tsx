...
const CommunityHub: React.FC<any> = ({ type }) => {
  ...
  // TODO: we romve loading for now
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const { error, value } = useFetch(
    `http://localhost:3001/${type.slice(0, -1)}/all/${user_id}`,
    {},
    [type]
  );
  ...
  const handleBlockFriend = async (e: React.MouseEvent<HTMLAnchorElement>, friend_id: string) => {
    e.preventDefault();
      try {
            const response = await fetch(`http://localhost:3001/friend/${user_id}/block/${friend_id}`, {
            method: "POST", 
          });
          ...
          if (res?.statusCode !== undefined) {
            console.log('heeeeeeeeeeerrrrrrrrrrreeeeeeeeeeeeeeeeee1111111111');
            throw new Error('An error occurred, Please try again.');
          }
          
          if (!response.ok){
            console.log('heeeeeeeeeeerrrrrrrrrrreeeeeeeeeeeeeeeeee2222222222');
  ...
  useEffect(() => {
    if (type === 'friends') {
      chatSocket.on("removeFriend", (removedFriendData: any) => {
        if (removedFriendData) {
          setData((prevValue: any) => {return {friends: prevValue.friends.filter((item: any) => item.id !== removedFriendData?.id)}})
        };
      });
    }
  }, [])
