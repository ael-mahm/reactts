...
const BannedMembers : React.FC<BannedMembersProps> = ({ data, setData }) => {
      ...
      const handleUnBan = async (user_id: string, unbannedId: string) => {
        if (!window.confirm('Are you sure you want to unban had khona?')) return;
        try {
              const response = await fetch(`http://localhost:3001/room/${user_id}/${group_id}/unban`, {
                method: "POST",
              ...
              if (res) {
                setData((prevValue: any) => {
                  if (!prevValue) return prevValue; // Handle case where prevValue is undefined TODO: hadi ghi tzadt, more checks
                  return {
