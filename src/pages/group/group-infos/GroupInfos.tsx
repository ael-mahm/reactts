...
interface GroupInfosProps {
  ...
  members_requests: any; // Define the type of members_requests if known
}
...
const GroupInfos: React.FC<GroupInfosProps> = ({ groupInfos, members_requests }) => {
  ...
  const handleDeleteGroup = async (group_id: string) => {
    try {
        const response = await fetch(`http://localhost:3001/room/${user_id}/${group_id}/delete`, {
          method: "DELETE",
  ...
  const handleJoinGroup = async (group_id: string) => {
    ...
    try {
            let response = await fetch(`http://localhost:3001/room/joinRoom`, {
            method: "POST",
      ...
      if (groupType === "public") {
        return (
          <div className="actions-buttons" data-membership="visitor">
            ...
            {/* CHECK OUT IF THE GROUP TYPE IS PRIVATE AND THE MEMBERSHIP REQUEST WAS SENT */}
