...
interface GroupSettingsProps {
  members: any; // You can replace 'any' with a more specific type
  admins: any; // You can replace 'any' with a more specific type
  componentFor: any; // You can replace 'any' with a more specific type
  groupInfos: any; // You can replace 'any' with a more specific type
  setData: (data: any) => void; // You can replace 'any' with a more specific type
}
...
const GroupSettings: React.FC<GroupSettingsProps> = ({ members, admins, componentFor, groupInfos, setData }) => {
  // hna kant HTMLInputElement | null
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (componentFor === 'create') {
      if (passwordRef?.current && groupNameRef?.current && groupTypeRef?.current){
        ...
        setRoomCredentials({url:`http://localhost:3001/room/createRoom`, room})
      }
    }
    ...
    if (componentFor === 'edit') {
      if (passwordRef?.current && groupNameRef?.current && groupTypeRef?.current){
        ...
        setRoomCredentials({url:`http://localhost:3001/room/${user_id}/${groupInfos.id}/updateRoom`, room})
      }
    }
  }
  ...
  useEffect(() => {
    if (groupSettingsRef.current && componentFor === "create") {
      ...
      // console.log(groupSettingsRef.current.offsetHeight);
    }
