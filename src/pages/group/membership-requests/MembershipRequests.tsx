...
const MembershipRequests: React.FC<MembershipRequestsProps>  = ({ data , setData}) => {
  ...
  const { connectedUser } = useContext(ConnectedContext);
      ...
      const handleAccept = async (user_id: string, userTwo: string) => {
        if (!window.confirm('Are you sure you want to Accept had khona?')) return; 
        try {
                  const response = await fetch(`http://localhost:3001/room/${user_id}/${group_id}/accept`, {
                  method: "POST",
    ...
    const handleDecline = async (user_id: string, userTwo: string) => {
      if (!window.confirm('Are you sure you want to Decline had khona?')) return;
      try {
              const response = await fetch(`http://localhost:3001/room/${user_id}/${group_id}/decline`, {
                  method: "POST",
                  body: JSON.stringify({ adminId: user_id, roomId: group_id, userTwo: userTwo }),
                  headers: {
                    "Content-Type": "application/json",
