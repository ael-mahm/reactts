...
const Role: React.FC<any> = ({ data, setData }) => {
  ...
  const handleMute = async (user_id: string, mutedUser: string, type: string) => {
    if (!window.confirm('Are you sure you want to mute had bnadm?')) return;
    try {
            const response = await fetch(`http://localhost:3001/room/${user_id}/${group_id}/mute`, {
              method: "POST",
  ...
  const handleBan = async (user_id: string, bannedId: string, type: string) => {
    if (!window.confirm('Are you sure you want to set had khona?')) return; 
    try {
              const response = await fetch(`http://localhost:3001/room/${user_id}/${group_id}/ban`, {
              method: "POST",
  ...
  const handleSetAmin = async (user_id: string, newAdmin: any) => {
      if (!window.confirm('Are you sure you want to set had bnadm as admin?')) return; 
      try {
              const response = await fetch(`http://localhost:3001/room/${user_id}/${group_id}/setAdmin`, {
            method: "POST",
  ...
  const handleKick = async (user_id: string, kickedUser: any) => {
      // if (!modalConfirmation) return; //THIS A MAIN LINE FOR THE MODAL CONFIRMATION 
      if (!window.confirm('Are you sure you want to delete had bnadm?')) return; 
      try {
            const response = await fetch(`http://localhost:3001/room/${user_id}/${group_id}/kick`, {
            method: "POST",
